...
function addRecent(journal, entry) {
...
function formatBattleBriefing(state, analysis, journal, options = {}) {
  const battle = state.battle;
  const { player, enemy } = battle;
  const enemyLine = options.fairPlay ? `${enemy.species} L${enemy.level}, roughly ${enemy.hpPercent}% HP, ${enemy.status}` : `${enemy.species} L${enemy.level} ${enemy.hp}/${enemy.maxHp}HP (${enemy.hpPercent}%), ${enemy.status}, types ${enemy.types.filter((t) => t !== "UNKNOWN").join("/")}, stats atk ${enemy.stats?.attack} def ${enemy.stats?.defense} spd ${enemy.stats?.speed} spc ${enemy.stats?.special}`;
  ...
  const threatLines = analysis.incoming.length ? analysis.incoming.map(
    (threat) => `  ${threat.move} (${threat.type}) \u2192 ~${threat.damage.typical} dmg to you (${threat.damage.min}-${threat.damage.max})${threat.canKo ? " \u2014 CAN KNOCK YOU OUT" : ""}`
  ) : ["  (unknown \u2014 you have not seen its moves yet)"];
  const stages = (s) => Object.entries(s).filter(([, v]) => v !== 0).map(([k, v]) => `${k} ${v > 0 ? "+" : ""}${v}`).join(", ") || "none";
...
BAG: ${state.world.bag.map((b) => `${b.item} x${b.count}${BATTLE_ITEMS[b.item] ? ` (${BATTLE_ITEMS[b.item]})` : ""}`).join(", ") || "(empty)"}
...
var BATTLE_INSTRUCTIONS = `Pick this turn's action in the battle below.
...
function fallbackBattleDecision(analysis) {
  const usable = analysis.moves.filter((move) => move.pp > 0 && move.effectiveness > 0);
  const best = usable[0] ?? analysis.moves[0];
  return {
    reasoning: best ? `Fallback heuristic: ${best.name} has the best expected damage this turn.` : "Fallback heuristic: no usable moves, attacking anyway.",
    action: "fight",
...
${briefing}`,
    ...
    console.error(`[jev] battle model call failed, falling back: ${error.message}`);
    return { decision: fallbackBattleDecision(analysis), usedFallback: true };
  }
}
function sanitize(decision, analysis) {
...
var Controller = class {
   ...
   */
  cursorLabel(screen = this.screen()) {
  ...
  waitFor(predicate, maxFrames = 600) {
  ...
  useMove(moveName2) {
    ...
    const target = moveName2.toUpperCase().replace(/[^A-Z0-9]/g, "");
  ...
  useItem(itemName2) {
    ...
    this.press("A");
    this.#gb.advance(30);
    const target = itemName2.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 5);
    const found = this.navigateList(
